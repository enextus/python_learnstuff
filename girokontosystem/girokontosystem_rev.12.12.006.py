    ...
    def check_action(self, x):
        ...
        if self.login:
            if x == '1': # 1 - Kontostand
                ...
                for i in k:
                    if str(self.login_kontonummer) == str(i.get_kontonummer()):
                        ...
                        return self.print_something(part1, part2)
            elif x == '2': # 2 - Einzahlen
                k = self.get_konten_liste()
                for i in k:
                    if str(self.login_kontonummer) == str(i.get_kontonummer()):
                ...
                self.make_clear_with_sleep()
                return self.einlogen()
            elif x == '4': # 4 - Überweisen
                k = self.get_konten_liste()
                
                for i in k:
                    if str(self.login_kontonummer) != str(i.get_kontonummer()):
                        print('Für eine Überweisung vorhandene Konten: ', i.get_kontonummer())
                        q = input("Wieviel Geld möchten Sie überweisen? ")
                        kn = input("Wie lautet das Ziel-Kontonummer? ")

                        for i in k:
                            if str(i.get_kontonummer()) == str(kn):
                                self.obj = i

                        self.obj.make_transfer(self.get_obj(), q)
            ...
            if x == '1':
                ...
                return self.print_something(x, self.get_konten_liste())
            elif x == '5':
                self.make_clear_with_sleep()
                x = "Die Konten als Objekte sehen so aus:\n"
                return self.print_something(x, self.get_konten_liste())
...
class Konto(Inhaber, Inputdata, App_controller):
    ...
    def make_transfer(self, transfer_summe):
        ...
        if transfer_summe <= self.kontostand and (self.get_tages_abzuege() + transfer_summe) <= self.get_tages_limit_fuer_abzuege():
            ...
            # obj.kontostand += transfer_summe
            # obj.tagesumsatz += transfer_summe
